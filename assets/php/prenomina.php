...
if ($periodo == 1) {
    $date1 = date("Y-m-d", strtotime("01-" . $mes . "-" . $ano));
    $date2 = date("Y-m-d", strtotime("15-" . $mes . "-" . $ano));
    $texto = mb_strtoupper(strftime(" 1RA DE %B %G", strtotime($date1)));
} else {
    $date1 = date("Y-m-d", strtotime("16-" . $mes . "-" . $ano));
    $month = $ano . '-' . $mes;
    $aux = date('Y-m-d', strtotime("{$month} + 1 month"));
    $date2 = date('Y-m-d', strtotime("{$aux} - 1 day"));
    $texto = mb_strtoupper(strftime(" 2DA DE %B %G", strtotime($date1)));
}

$descuentos = [];

$sql = "SELECT * FROM Usuario WHERE categoria = 'user' AND STR_TO_DATE(fechaRelLab,'%d/%m/%Y') <= '" . $date2 . "'";
$consulta = mysqli_query($conexion, $sql);
while ($usuario = mysqli_fetch_row($consulta)) {
    $fecha_inicio = date("Y-m-d", strtotime(str_replace('/', '-', $usuario[10])));
    if ($usuario[7] === 'alta') {
        if (($fecha_inicio >= $date1) && ($fecha_inicio <= $date2)) {
        ...
        if($fecha_baja[1] == 1){
            ...
            if (($fecha_baja >= $date1) && ($fecha_inicio <= $date2)) {
    ...
    $sql2 = "SELECT * FROM Permiso WHERE RFC = '" . $usuario[8] . "' AND categoria = 1 AND del <= '" . $date2 . "'";
    if (($consulta2 = mysqli_query($conexion, $sql2)) && (mysqli_num_rows($consulta2) > 0)) {
        while ($resultado1 = mysqli_fetch_array($consulta2)) {
            if ($resultado1['al'] >= $date2) {
                if ($resultado1['del'] < $date1) {
                    $fecha1 = $date1;
                } else {
    ...
    if ($descontados_permiso > 15) 
        $descontados_permiso = 15;
...
$sql = "SELECT Usuario.id_usuario, Usuario.nombre, Usuario.CURP, Usuario.RFC, Usuario.puesto, Usuario.departamento, Usuario.fechaRelLab, Baja.fecha, Baja.razon FROM Baja INNER JOIN Usuario ON Baja.RFC = Usuario.RFC WHERE Baja.fecha >= '" . $date1 . "' AND Baja.fecha <= '" . $date2 . "'";
$consulta = mysqli_query($conexion, $sql);
$i = 3;
if ($consulta && (mysqli_num_rows($consulta) > 0)) {
    while ($res = mysqli_fetch_array($consulta)) {
...
$sql = "SELECT id_usuario, nombre, CURP, RFC, puesto, departamento, fechaRelLab FROM Usuario WHERE STR_TO_DATE(fechaRelLab,'%d/%m/%Y') >= '" . $date1 . "' AND STR_TO_DATE(fechaRelLab,'%d/%m/%Y') <= '" . $date2 . "'";
$consulta = mysqli_query($conexion, $sql);
$i = 3;
if ($consulta && (mysqli_num_rows($consulta) > 0)) {
    while ($res = mysqli_fetch_array($consulta)) {
...
$sql = "SELECT id_usuario, nombre, CURP, RFC, puesto, departamento, fechaRelLab FROM Usuario WHERE STR_TO_DATE(fechaRelLab,'%d/%m/%Y') <= '" . $date2 . "'";
$consulta = mysqli_query($conexion, $sql);
$i = 3;
if ($consulta && (mysqli_num_rows($consulta) > 0)) {
    while ($res = mysqli_fetch_array($consulta)) {
...
WHERE Permiso.al >= '" . $date1 . "' AND Permiso.del <= '".$date2."' AND Permiso.categoria = 0 ORDER BY Permiso.RFC ASC";
$consulta = mysqli_query($conexion, $sql);
$i = 3;
if ($consulta && (mysqli_num_rows($consulta) > 0)) {
    while ($res = mysqli_fetch_array($consulta)) {
...
$sql = "SELECT Usuario.id_usuario, Usuario.nombre, Usuario.CURP, Usuario.RFC, Usuario.puesto, Usuario.departamento, Usuario.fechaRelLab,
Permiso.del, Permiso.al, Permiso.dias, Permiso.descripcion FROM Permiso INNER JOIN Usuario ON Permiso.RFC = Usuario.RFC
WHERE Permiso.del <= '" . $date2 . "' AND Permiso.categoria = 1 ORDER BY Permiso.RFC ASC";
$consulta = mysqli_query($conexion, $sql);
$i = 3;
if ($consulta && (mysqli_num_rows($consulta) > 0)) {
    while ($res = mysqli_fetch_array($consulta)) {
...
$sql = "SELECT * FROM Usuario WHERE categoria = 'user' AND tipoTrabajador IN('HONORARIOS','EVENTUAL') AND STR_TO_DATE(fechaRelLab,'%d/%m/%Y') <= '" . $date2 . "' ORDER BY departamento";
$consulta = mysqli_query($conexion, $sql);
$i = 3;
$departamentos = [];
if ($consulta && (mysqli_num_rows($consulta) > 0)) {
    ...
    while ($res = mysqli_fetch_array($consulta)) {
        $bandera = false;
        if ($res[7] === 'baja') {
            $sql1 = "SELECT RFC FROM Baja WHERE fecha >= '" . $date1 . "' AND fecha <= '" . $date2 . "' AND RFC = '" . $res[8]."'";
            $consulta1 = mysqli_query($conexion, $sql1);
...
AND STR_TO_DATE(fechaRelLab,'%d/%m/%Y') <= '" . $date2 . "' ORDER BY departamento";
$consulta = mysqli_query($conexion, $sql);
$i = 3;
$departamentos = [];
if ($consulta && (mysqli_num_rows($consulta) > 0)) {
