...
if ($condicion == 0) {
    $sql1 = "SELECT * FROM Chat WHERE RFC LIKE '%" . $texto . "%' OR RFC = (SELECT RFC FROM Usuario WHERE nombre LIKE '%" . $texto . "%' LIMIT 1) OR titulo LIKE '%" . $texto . "%' OR fecha LIKE '%" . $texto . "%' ORDER BY id_chat DESC";
} else {
    $sql1 = "SELECT * FROM Chat WHERE id_chat = " . $texto;
}
