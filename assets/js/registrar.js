...
$(document).ready(function () {
                ...
                success: function (data) {
                  if (data == 1) {
                    ...
                    md.showNotification(
                      "top",
                      "right",
                      "Error el empleado no fue registrado."
                    );
                  }
                },
              });
  ...
  $(document).on("change", "#input-foto", function () {
    $.blockUI({
      message:
        "<div class='circulo'></div><h5>Cargando foto de perfil ...</h5>",
    });
    var formData = new FormData();
    var files = $(this)[0].files[0];
    formData.append("file", files);
    ...
    if (data.success) {
      $.ajax({
        ...
        success: function (html) {
          ...
          $.post(
            "assets/php/fecha_movimiento.php",
            {
              id: id,
            },
            function (datos) {
          ...
          $("#form-baja").submit(function (e) {
            ...
            $("#modal .modal-body").html(
              "¿Seguro que quiere dar de baja a " + $("#nombre").text() + "?"
            );
            mostrar_modal();
...
function password(id) {
  ...
  $("#modal .modal-body").html(
    "¿Restablecer contraseña del usuario a la predeterminada?"
  );
  mostrar_modal();
  $("#modal_aceptar")
    .off()
    ...
    success: function (html) {
      ...
      if (html == 1) {
        ...
        md.showNotification(
          "top",
          "right",
          "Espere 24 hrs para dar de baja a este empleado."
        );
      } else {
        md.showNotification(
          "top",
          "right",
          "No fue posible dar de baja a este empleado."
        );
      }
    },
  });
    ...
    if (data.success) {
      $.ajax({
        ...
        success: function (html) {
          ...
          $.post(
            "assets/php/fecha_reingreso.php",
            {
              id: id,
            },
            function (datos) {
                    ...
                    success: function (data) {
                      ...
                      if (data == 1) {
                        ...
                        md.showNotification(
                          "top",
                          "right",
                          "No fue posible dar de alta a este empleado."
                        );
                      }
                    },
                  });
        ...
        $.ajax({
          ...
          success: function (html) {
            ...
            $("#categoria").change(function () {
              ...
              } else $("#materno").prop("disabled", false);
            });
                    ...
                    success: function (html) {
                      ...
                      if (html == 0) {
                        md.showNotification(
                          "top",
                          "right",
                          "Licencia agregada correctamente."
                        );
                        verPermisos(id);
                      } else {
                        md.showNotification(
        ...
        if (data.estado == 1) {
          ...
          $("#file").change(function () {
            if ($("#file").val() !== "") {
              $.blockUI({
                message:
                  "<div class='circulo'></div><h5>Cargando archivo ...</h5>",
              });
...
function ventanaRegresar(tabla, condicion, id, usuario) {
  if (tabla === "Permiso") {
    if (condicion == 0) verPermisos(usuario);
    else if (condicion == 1) detalle(id);
  } else if (tabla === "Vacacion") {
    if (condicion == 0) verVacaciones(usuario);
    else if (condicion == 1) detalle_vacacion(id);
  } else if (tabla === "Movimiento") {
    if (condicion == 0) verMovimientos(usuario);
    else if (condicion == 1) detalle_movimiento(id);
  } else if (tabla === "Descuento") {
    if (condicion == 0) verDescuentos(usuario);
    else if (condicion == 1) detalle_descuento(id);
  } else if (tabla === "Pase") {
    if (condicion == 0) verPases(usuario);
    else if (condicion == 1) detalle_pase(id);
  } else if (tabla === "Gastos") {
    if (condicion == 0) verGastos(usuario);
    else if (condicion == 1) detalle_gastos(id);
  } else if (tabla === "Historial") {
    if (condicion == 0) verHistorial(usuario);
    else if (condicion == 1) detalle_historial(id);
  }
}
                    ...
                    success: function (html) {
                      if (html == 0) {
                        md.showNotification(
                          "top",
                          "right",
                          "Vacaciones agregadas correctamente."
                        );
                        verVacaciones(id);
                      } else {
                        md.showNotification(
...
function detalle_vacacion(id) {
  $.post(
    "assets/php/detalleVacacion.php",
    {
      id: id,
    },
    function (datos) {
      ...
      $("#fecha").datepicker({
        ...
        onRenderCell: function (date, cellType) {
          ...
          if (
            cellType == "day" &&
            comprobarFecha(data.fecha2, data.fecha3, date)
          ) {
            return {
...
function detalle_movimiento(id) {
  $.post(
    "assets/php/detalleMovimiento.php",
    {
      id: id,
    },
    function (html) {
...
function borrar_movimiento(id) {
  $.post(
    "assets/php/comprobarMovimiento.php",
    {
      id: id,
    },
    function (salida) {
                  ...
                  success: function (html) {
                    ...
                    if (html == 0) {
                      md.showNotification(
                        "top",
                        "right",
                        "Gastos médicos agregados correctamente."
                      );
                      verGastos(id);
                    } else {
                      md.showNotification(
...
function detalle_descuento(id) {
  $.post(
    "assets/php/detalleDescuento.php",
    {
      id: id,
    },
    function (datos) {
                  ...
                  success: function (html) {
                    if (html == 1) {
                      ...
                      md.showNotification(
                        "top",
                        "right",
                        "Deber especificar una fecha y hora."
                      );
                    } else {
        ...
        myDropzone.on("queuecomplete", function (file) {
          if (bandera) {
            md.showNotification(
              "top",
              "right",
              "Se han cargado " + correctos + " archivos de " + total
            );
            $(".btn_subir").prop("disabled", true);
  ...
  if (ext != "png" || ext != "jpeg" || ext != "jpg") {
    switch (ext) {
      case "pdf":
        $(file.previewElement)
          .find(".dz-image img")
          .attr("src", "assets/img/icons/pdf.png");
        break;
      case "xlsx":
        $(file.previewElement)
          .find(".dz-image img")
